 ...
 * - Reporta errores léxicos con contexto
 * - Mantiene trazabilidad de línea y columna
 */
...
class Lexer {
    ...
    tokenize() {
        while (this.pos < this.input.length) {
            ...
            // Palabras reservadas
            if (this._match(/^(TORNEO|EQUIPOS|ELIMINACION|fase|partido|goleador)\b/iy, "RESERVADA")) continue;

            // Símbolos y conectores
            if (this._match(/^(\{|\}|\[|\]|:|,| vs) \b /y, "SIMBOLO")) continue;

            // Cadena con cierre obligatorio
            const cadenaRegex = /^"([^"\\]|\\.)* "/y;
            ...
            // Números
            if (this._match(/^\d+/y, "NUMERO")) continue;

            // Identificadores
            if (this._match(/^[A-Za-z_][A-Za-z0-9_]*/y, "IDENTIFICADOR")) continue;

            // Si nada coincide, es error léxico
            this._recordError();
        }
